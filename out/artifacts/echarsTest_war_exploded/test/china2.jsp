...
<script>
    ...
    chart.on('click', function (params) {
        ...
        if (params.name in provinces || params.name in special) {
           ...
           // backMap = params.seriesName;
            ...
            //如果是 区级别 显示区级地图
            /* alert("区 县 params.name"+params.name);
             alert("区 县 params.seriesName"+params.seriesName);*/
    ...
    function goBack() {
        ...
        if (backMap == 'china') {
            ...
            //backMap = xx 市
            $.getJSON('/json/city/' + cityMap[backMap] + '.json', function (data) {
                echarts.registerMap(p.name, data);
                var d = [];
                for (var i = 0; i < data.features.length; i++) {
                    d.push({
