...
function getTableDefaultField(row, isHas, isList) {
    var sheet = $SP.getActiveSheet(spread);
    let ADMDIV = speardUtil.getPms("USER_admdiv");
    let MOF_DIV_CODE = speardUtil.getPms("USER_admdivCode");
    let AGENCY_ID = speardUtil.getPms("USER_deptid");
    let AGENCY_CODE = speardUtil.getPms("USER_deptCode");
...
function insertRow_after(params, row) {
    ...
    let data = getTableDefaultField(row, null, true);
    data = $.extend(data, {
