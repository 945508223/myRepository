...
function bindEventForMap_(mapIns) {
    mapIns.on('mouseover', function (params) {
       if(mapCfg_.intervalKey) clearInterval(mapCfg_.intervalKey);
        mapIns.dispatchAction({
            type: 'downplay',
            seriesIndex: 0,
            dataIndex: mapCfg_.index_ == 0 ? 10 : mapCfg_.index_ - 1
...
/*
水球图数据源
$DV.div($DV.getCell("exec1",0,"PAY_AMT"),$DV.getCell("budget1",0,""));
$DV.div($DV.getCell("budget1",0,"AMT"),($DV.getCell("budget1",0,"AMT")+$DV.getCell("budget2",0,"AMT")))
$DV.div($DV.getCell("budget2",0,"AMT"),($DV.getCell("budget1",0,"AMT")+$DV.getCell("budget2",0,"AMT")))
*/
