...
<div class="tool-group">
    <span style="margin-right: 15px;">CSS在线编辑器</span>
    ...
    <div id="right_btn" style="margin-left: 59%">
...
<div id="splitContainer">
    <div>
        <div id="leftSplitter" style="height:100%">
...
<script type="text/javascript">
    ...
    //GUID:加载页面css,如果为页面级css需要从数据库中取
    var guid = YCDCommon.Win.getUrlParam("GUID");
    //ID:加载事件css,事件css直接从当前页面id对应的对象中找
    ...
    var css_Text =(id)?parent.VUECFG.formObj[id].info[name]:null;

    if(guid){
        getJsValueByGuid()
    }else {
        if(css_Text)
            css_editor.setValue(css_Text);    //给代码框赋值
    }
    ...
    //查询页面脚本css
    function getCssValueByGuid(){
        debugger
        var col="GUID,CSS_TEXT";
        if(type==="pubcss"){
        ...
        if(data){
            ptype="edit"
            var base = new Base64();
            var cssValue=(data.JS_TEXT)?base.decode(data.JS_TEXT):"";
            if(cssValue&&cssValue!=""){
                css_editor.setValue(cssValue);    //给代码框赋值
            }
        }
    }
    //保存css
    function save_css(){
        debugger
        var base = new Base64();
        var cssValue=css_editor.getValue();
        if(guid&&type==="private"){
            ...
            parent.$DS.saveTable(parent.VUECFG.appId,ptype,datas,"DM_FORM_PAGEJS","GUID",function (data) {
                debugger
                parent.$DS.setPro("PAGE","ds_pagecss","csseditor", "【脚本】")
                close_();
                parent.alert("保存成功!")
            })
        }else if(guid&&type==="pubcss"){
            debugger
            var datas={
            ...
            parent.$DS.setPro(id,name,(ispro=="true")?"csseditor":"input",cssValue);
            close_();
    ...
    function addLibJs(e) {
        ...
        let html = generateHtmlTemplate();//html_editor.getValue();
        fd.onload = function() {
    ...
    function selectTemplate(e) {
        ...
        setCommonTemplete(css_editor,value)
        //css_editor.getDoc().setValue($("#"+e.target.value).html());
    }
</script>
<!--模板-->
<script id="template1" type="text/css">
    .cs{
        width: 10px;
    }
</script>

<script id="template2" type="text/css">
    a{
        height: 10px;
    }
</script>
