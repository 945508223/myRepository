...
function zbfp(dom) {
    ...
    var year = $(dom).attr("year");
    if (amount) {
        amount = parseFloat(amount).toFixed(2).replace(/\d{1,3}(?=(\d{3})+(\.\d*)?$)/g, '$&,');
    }
    $DS.showPage('freeFromView.jsp?PAGEID=3EA2482FAF224AB98628E3D875E33D71&PAGETITLE=【0104】指标分配保存&APPID=BMP&GUID=' + guid + "&amount=" + amount + '&dis_amount=' + dis_amount + '&year=' + year, '指标分配', '80%', '80%')
}
...
function btn_delindi(obj) {
    ...
    if (!$DS.getVal("指标列表")) {
        ...
        $DS.util.confirm(vm, "您确认要删除【" + $DS.getPms("P_INDI").BGT_DOC_TITLE + "】指标吗？", function () {
            //取fastdfs服务器文件列表
            var indi_GUID = $DS.getVal("指标列表")[0].GUID;
            var filelist = $DS.selectAll(VUECFG.appId, "RURAL_INDI_POL", ["GUID", "POLURL"], "GUID", {filter1: "and INDI_ID='" + indi_GUID + "'"});
            //删除数据库数据
...
var pageObj = {
    centerwidth: $DS.getCtrl("指标列表容器").info.ds_width,
    leftWidth: $DS.getCtrl("项目分类容器").info.ds_width,
    rightWidth: $DS.getCtrl("TAB页").info.ds_width,
...
function setMenuRowWidth_MenuHeight_InputIsShow() {
    $DS.getCtrl("项目分类容器").info.ds_width = pageObj.leftWidth;//导航栏_搜索框容器
...
function setZbRowWidth() {
    $DS.getCtrl("指标列表容器").info.ds_width = pageObj.centerwidth;//中间容器
}


//设置tab页宽度
function setTabRowWidth() {
    $DS.getCtrl("TAB页").info.ds_width = pageObj.rightWidth;
}
 ...
 * 是否显示指标分配列 县级不显示
 */
function isShowZBFPCol() {
    var level = $DS.getPms("USER_ADMDIVLEVEL");
    if (level == "4") {
        var source = $DS.getSource("指标列表");
        for (let i = 0; i < source.columns.length; i++) {
            if (source.columns[i].FIELD_NAME == "ZBFP") {
                source.columns.splice(i, 1);
                break;
            }
        }
        var ctrl = $DS.getCtrl("指标列表").info;
        for (let i = 0; i < ctrl.ds_selectcolumns.length; i++) {
            if(ctrl.ds_selectcolumns[i].FIELD_NAME=="ZBFP"){
                ctrl.ds_selectcolumns.splice(i,1);
                break;
            }
