...
function downTask_ZYL() {
    ...
    var detelWin = $tabs.getSubWindow("TABS_区划任务列表", 1);
    ...
    for (let i = 0; i < selectRes.length; i++) {
        if (selectRes.GUID == $DS.getPms("USER_UPADMDIV")) {
            continue;
        }
        let newItem = {};
    ...
    //rows.inserted.length === 0 && rows.updated.length === 0 && rows.deleted.length
...
function deleteTask() {
    ...
    $DS.util.confirm(vm, "您确认要删除【" + selectTask.TASKNAME + "】任务吗？", function () {
        ...
        var sql2 = "delete from RURAL_TASK_DETAIL where TASKADMDIV='" + selectTask.GUID + "'";
        sqls.push(sql1);
        sqls.push(sql2);
...
//设置单位
function setDanwei(){
    var name = $DS.getPms("USER_admdivName");
    var ctrlInfo = $DS.getCtrl("INPUT_填表单位").info;
    ctrlInfo.ds_default = name.replace("本级")+"财政局";
}