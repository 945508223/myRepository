...
//---------------------------------------------------根据不同条件生成预测数据-------------------------------------------------------------------------
/**
 * 根据iframename 获取iframe window对象
 * @param iframeName
 */
function getIframeWin(iframeName) {
    return $DV.getEl(iframeName).find("iframe")[0].contentWindow;
}
...
function loadTableDataByNewData() {
    var win = parent.contentWindow.getIframeWin("简单表格2")
    win.$DV.loadEl("简单表格2")
}
...
var grid1Win = getIframeWin("网页框1")
var grid3Win = getIframeWin("网页框3")
grid1Win.reloadTable()
grid3Win.reloadTable()