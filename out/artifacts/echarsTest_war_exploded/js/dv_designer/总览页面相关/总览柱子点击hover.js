...
/*var loadReady = setInterval(function () {
    if($DV.isLoaded){
        bindClick("ctrl_623120-175614457", 0, "收入增幅");
        bindClick("ctrl_622120-194039324", 0, "收入增幅");

        bindClick("ctrl_622120-194037116", 1, "一般公共预算收入");
        bindClick("ctrl_623120-18313424", 1, "一般公共预算收入");

        bindClick("ctrl_623120-175949120", 2, "税占比");
        ...
        window.clearInterval(loadReady)
    }
},200);*/
...
function changeData(data) {
    ...
    var newLabel;
    var newTooltip;
    var whichDim; //dv_visualmap_dimension GEN_PUB_BUD_AMT_DESC
    switch (data) {
        case "收入增幅":
            if (pageId === "B2CC35E3D8A03A75E0535501A8C01276") { //省

                newLabel = "{YBGGYS_YEARZF_DESC}";
                newTooltip = "{MOF_DIV_NAME},排名：{YBGGYS_YEARZF_DESC},收入增幅：{GEN_PUB_BUD_YEAR_GROPER}%";
                whichDim = "YBGGYS_YEARZF_DESC";

            } else if (pageId === "B317534EC7F134F8E0535501A8C09B14") {//苏南

                newLabel = "第{GEN_PUB_BUD_YEAR_GROPER_DESC}名";
                newTooltip = "{MOF_DIV_NAME},排名：{GEN_PUB_BUD_YEAR_GROPER_DESC},收入增幅：{GEN_PUB_BUD_YEAR_GROPER}%";
                whichDim = "GEN_PUB_BUD_YEAR_GROPER_DESC";
        ...
        case "税占比":
            if (pageId === "B2CC35E3D8A03A75E0535501A8C01276") {
                ...
                whichDim = "SZB_YEAR_PER_DESC";

            } else if (pageId === "B317534EC7F134F8E0535501A8C09B14") {

                newLabel = "第{TAX_GENPUBDUD_YEAR_PER_DESC}名";
                newTooltip = "{MOF_DIV_NAME},排名：{TAX_GENPUBDUD_YEAR_PER_DESC},税占比：{TAX_GENPUBDUD_YEAR_PER}%";
                whichDim = "TAX_GENPUBDUD_YEAR_PER_DESC";
    ...
    if (pageId !== "B31A9A3C039A4576E0535501A8C06824") {//以地图为映射的镇江
        mapEl.attr("dv_effectscatterlabel", newLabel);
        mapEl.attr("dv_effecttooltip", newTooltip);
        mapEl.attr("dv_visualmap_dimension", whichDim);
    } else {//以散点为映射
        mapEl.attr("dv_series_itemstyle_label_formatter", newLabel);
        mapEl.attr("dv_series_tooltip_formatter", newTooltip);
        mapEl.attr("dv_visualmap_dimension", whichDim);
    }
