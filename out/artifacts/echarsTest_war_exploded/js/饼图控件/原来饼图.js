...
function buidPieOptionByName(element, dataSourceElement, result, index) {
    ...
    if (result && result.length > 0) {
        ...
        //获取第二组数据
        //var ncode_dim2 = (getOrderItems(dragDatas[$(dataSourceElement).attr("id") + "||dv_dim"])[1]["NCODE"]).toUpperCase();
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            item.value = row[ncode_meas] == 0 ? null : row[ncode_meas];
            item.name = row[ncode_dim + "NAME"] || row[ncode_dim];
            item.cid = row[ncode_dim + "ID"];
            item.name2 = row[ncode_dim2 + "NAME"] || row[ncode_dim2];
