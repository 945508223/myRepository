...
function sendUserMsgForReport() {
    ...
    var result = $DS.massage.sendUserMsg(VUECFG.appId, {
        ...
        receiveMenuId: "006003003",//消息接收菜单 预算汇总审核
        receiveADMDIV: upRes[0].SUPERGUID,//消息接收区划
        receiveUserType: "1",//接收用户类型
        url: "F4C9F8FFC8A141798DA4EF4B3342DC10,F28DE3A8041E476696F9C4F60DEA300D",//全省预算,预算汇总审核
...
function sendUserMsgForTuiHui() {
    ...
    var result = $DS.massage.sendUserMsg(VUECFG.appId, {
        ...
        appId: VUECFG.appId,//appId
        receiveMenuId: "006003002",//消息接收菜单 预算汇总上报
...
function btn_complete_complete(obj) {
    ...
    if (myParent == "isSummary") {
        if (admdivLevel == "2") {//用户是省级
            if (budgetLevel == "2" && endFlag != "1") {//数据是省
                //暂时关闭省级的全部通过按钮的显示
                /* obj.ds_button = "全部通过";
                 obj.ds_show = true;*/
            } else if (budgetLevel == "3" && endFlag != "1" && taskStatus != "42" && taskStatus == "223") {
                obj.ds_button = "审核通过";
                obj.ds_show = true;
            }
        } else if (admdivLevel == "3") {//用户是市级
            if (endFlag == "0" && taskStatus != "223" && taskStatus != "5" && taskStatus != "42") {//市
                //暂时关闭市级全部通过按钮的显示
                /*obj.ds_button = "全部通过";
                obj.ds_show = true;*/
