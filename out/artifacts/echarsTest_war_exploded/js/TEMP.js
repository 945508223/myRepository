        ...
        computed: {
            ...
            computed_data: function () {
                try {
                    ...
                    let showData = [];
                    //搜索数据
                    if (this.info.ds_enablesearch && this.isFilter && (this.keyWord ?? '') !== '') {
                        //todo 调用搜索方法 返回搜索数据
                    ...
                    if (this.info.ds_tree_grid && this.info.ds_grid) {
                        if (this.info.ds_row_id && this.info.ds_row_pid) {
                            showData = $DS.util.children(showData, this.info.ds_row_id, this.info.ds_row_pid, "children");
                            //设置分层汇总数据
...
function setGridData(id, val, cache) {
    ...
    if (val) {
        ctrl['ds_grid'] = "";
        ctrl['ds_grid'] = $DS.util.clone((val.rows) ? (val.rows) : val);
    } else if (source) {
        ...
        //表格数据
        ctrl['ds_grid'] = $DS.util.clone(source.sourceData.rows);
