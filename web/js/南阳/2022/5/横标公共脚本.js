 ...
 * @param guid 当前行guid
 * @param BFNum 补发月数 存在使用  否则根据执行月份计算
 */
...
function changePersonstaffOptions(params) {
    ...
    //修改参加工作时间 计算工龄
        ...
        setWorkYear(params)
    } else if (params.tag.fieldname == 'CHANGERESON') {
            ...
            try {
                ...
                if (result) {
                    if (errorData.length > 0) {
                        ...
                        parent.$DS.util.confirm(parent.window.vm, `请检查 ${errorData.join('，')} 行姓名或身份证号`, function () {
...
function formatImportData(val, tag, saveItem) {
    ...
    if (tag.isSelectCol == 'TRUE') {
        ...
        val = unions.find(item => {
            let cnName = item.ELE_NAME.split('-').length == 2 ? item.ELE_NAME.split('-')[1] : item.ELE_NAME;
            //中文名称
            if (cnName == val) {
                return item;
            }
...
function getJobOptions(tag, saveItem) {
    ...
    if (saveItem.PERSONCAT == '1') {
        if (window.top.allUnion.XZJOB) {
            ...
            let sqlXZ = `SELECT  ELE_CODE , ELE_CODE||'-'||ELE_NAME AS ELE_NAME FROM ELE_UNION WHERE  ELE_CATALOG_ID IN('ZW','ZJ') ORDER BY ELE_CODE`
        ...
        if (window.top.allUnion.SYJOB) {
            ...
            let sqlSY = `SELECT  ELE_CODE ,ELE_CODE||'-'||ELE_NAME AS ELE_NAME FROM ELE_UNION WHERE  ELE_CATALOG_ID='JSDJ' ORDER BY ELE_CODE`
...
//行政人员 相关引用
    if (saveItem.PERSONCAT == '1') {
        if (window.top.allUnion.XZGZJB) {
            ...
            let sqlXZ = `SELECT  ELE_CODE ,ELE_CODE||'-'||ELE_NAME AS ELE_NAME FROM ELE_UNION WHERE  ELE_CATALOG_ID='GZJB' ORDER BY ELE_CODE`
