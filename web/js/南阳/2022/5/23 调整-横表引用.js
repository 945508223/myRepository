...
function loadAfterSetZwOptions() {
    ...
    try {
        //行政引用职务
        let sqlXZ = `SELECT  ELE_CODE AS value, ELE_CODE||'-'||ELE_NAME AS text FROM ELE_UNION WHERE  ELE_CATALOG_ID IN('ZW','ZJ')  ORDER BY ELE_CODE`
        //事业引用技术等级
        ...
        //行政人员 级别(岗位)档次
        let OPOSTGRADSql_xz = `SELECT  ELE_CODE AS value,ELE_CODE||'-'||ELE_NAME AS text FROM ELE_UNION WHERE  ELE_CATALOG_ID='GZJB' ORDER BY ELE_CODE`;
        //事业人员 级别(岗位)档次
        ...
        let oldZwCol = getCellPositonInRow(2, 'OLDJOBLEVEL')?.col || 31;//变动前技术等级
        let newZwCol = getCellPositonInRow(2, 'NEWJOBLEVEL')?.col || 45;//变动后技术等级
        let oldJob = getCellPositonInRow(2, 'OLDJOB')?.col || 21;//原职务技术登记
        let newJob = getCellPositonInRow(2, 'NEWJOB')?.col || 23;//现职务技术等级
        let PERSONSTAFFCol = getCellPositonInRow(2, 'PERSONSTAFF')?.col || 7;
        let fromCol = getCellPositonInRow(2, 'PERSONFROM')?.col || 6;
        let OLDOPOSTGRADCol = getCellPositonInRow(2, 'OLDOPOSTGRADCol')?.col || 34;
        let NEWOPOSTGRADCol = getCellPositonInRow(2, 'NEWOPOSTGRAD')?.col || 48;
