...
function setDefaultValue() {
    ...
    pageParams["saveObj"]["NY_PAYROLL"]["CHANGERESON"] = `' '`;
...
function pubSave_Y(keyFiled, keyVal) {
    ...
    for (let tableName of infoArr) {
        ...
        if (keyVal) {
            ...
            sqls = [`insert into ${tableName} (${insInfoArr.join(",")},PERSONID) values (${insValArr.join(",")},'${newGuid}')`];
            //新增人员同时向人员基本信息表存一条记录 审批通过执行
            let personInfoFields = ['IDNO', 'AGENCYID', 'AGENCYCODE', 'PERSONNAME', 'YEAR', 'TASKMONTH', 'SEX'];
            let insInfoArr_p = [], insValArr_p = [];
            personInfoFields.forEach(fieldName => {
                if (saveObj[tableName][fieldName]) {
                    insInfoArr_p.push(fieldName);
                    insValArr_p.push(saveObj[tableName][fieldName]);
                }
            });
            //let sql2 = `insert into NY_PERSONINFO (${insInfoArr_p.join(",")},PERSONID,ISXZ,ISDC,PERSONSTATUS) values (${insValArr_p.join(",")},'${newGuid}','1','2','3')`
            //sqls.push(sql2);
        }
