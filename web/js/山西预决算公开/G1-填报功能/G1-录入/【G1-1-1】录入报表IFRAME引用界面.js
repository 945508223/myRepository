...
function checkSuccessBF() {
    ...
    if (!checkStatus) {
        alert("报表未校验或校验未通过；请点击批量校验，查看校验结果!");
        return false;
    }
    ...
    if (currentYearRptId.length > 0 && currentYearRptId.length != checkStatus.length) {
        ...
        if (noOpenArr.length > 0) {
            ...
            alert(`以下报表存在未校验或校验未通过情况，请检查！<br>${rptNames.join("")}`);
            return false;
        }
    }
    //判断校验表中是否存在校验不通过的数据
    ...
    if (checkArr.length > 0) {
        ...
        result = $DS.selectBySql(VUECFG.appId, sql);
        let rptNames = result.result.map(item => `${item["ITEMNAME"]}<br>`);
        alert(`以下报表校验未通过，请点击校验按钮！<br>${rptNames.join("")}`);
        return false;
    }
    return true;
}
            ...
            for (let i = 0; i < tabCfgs.length; i++) {
                ...
                if (!pageInfo_[tabCfg.name] && !pageInfo_.loadingTab) {
                    ...
                    if (rptId) {
                        ...
                        let url = `${getProjectName()}/report/reportdesigner/lookreport/reportView.jsp?ylTb=${tabCfg.name}&isExcelRptToHtml=true&excelRptToHtmlId=${rptId}`;
                        //拼接参数
                        for (let pmsKey in tabsInfo_[tabCfg.name]) {
                            if(pmsKey=='updivid'&&tabsInfo_[tabCfg.name][pmsKey]=='#'){
                                tabsInfo_[tabCfg.name][pmsKey] = $DS.getPms('USER_admdiv')
                            }
...
function dealHtmlTable(htmlTable) {
    ...
    let {toHtmlWidth, toHtmlHeight, toHtmlhorizontally, toHtmlvertical, toHtmlPadding, toHtmlHDirection,toHtmlIsPaging, toHtmlTitle, toHtmlTitleLevel} = htmlTable;
 ...
 */
        ...
        let outDiv =
            `<div class="ds_ctrl">` +
            `<div id="innerHtmlTableContainer_${id}" style="float: left;padding:${toHtmlPadding};height: auto;width:100%;display:flex;` +
            `justify-content:${toHtmlhorizontally};align-items:${toHtmlvertical}">` +
            ` ${html}</div>`;
        //横向报表
        if (toHtmlHDirection == true)
        ...
        //分页标识
        if (toHtmlIsPaging)
            outDiv += `<div style="display:none;">=!@#%=</div>`;
        outDiv += `</div>`
        $("#outerHtmlTableContainer").append(outDiv);
        //添加标题相关标识
        if (toHtmlTitleLevel) {
            $('.ds_ctrl').attr('reportpdftitle', toHtmlTitleLevel);
            $('.ds_ctrl').attr('reportpdftext', toHtmlTitle);
        }
        $(`#innerHtmlTableContainer_${id} table`).css('height', toHtmlHeight).css('width', toHtmlWidth);
