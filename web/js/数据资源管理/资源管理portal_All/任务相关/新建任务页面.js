...
//--------------------资源管理相关--------------------------------------------------------------------
//初始化构建资源分类option
function initSourceTypeOption(info) {
    debugger
    //加载列option 用于保存和回显
    let typeData;
    if (!taskPageInfo.DSTYPEOPTIONS) {
        let appName = $DS.getPms('URL_APPNAME');
        let sql = `select GUID,SUPERGUID,ITEMNAME FROM SSO_T_RESOURCECAT WHERE APPNAME = '${appName}'`;
        let result = $DS.selectBySql('PORTAL', sql, '获取资源分类信息失败');
        typeData = result.result;
        taskPageInfo.DSTYPEOPTIONS = typeData.map(item => {
            let option = {};
    ...
    //加载树形资源分类数据 用于选择
    if (!taskPageInfo.DSTYPETREE) {
        taskPageInfo.DSTYPETREE = $DS.util.children(typeData, 'GUID', 'SUPERGUID', 'children');
    }

    let gridVm = window[info.ds_id + "_gridRef"];
    for (let col of gridVm.computed_columns) {
        if (col.id == 'DSTYPE') {
            col.fieldShowType = 'select';
            col.options = taskPageInfo.DSTYPEOPTIONS;
            break;
        }
    }
}
...
//打开树 设置资源分类
function openSourceType(col, row, cell) {
    debugger
    if (col.property == 'DSTYPE') {
        //配置
        let cfg = {
            node_key: "GUID",//必传
            props: {       //必传
            ...
            show_checkbox: false,
            current_node_key : row.DSTYPE?row.DSTYPE:'',
            default_expanded_keys: row.DSTYPE?[row.DSTYPE]:[],
            filterFields: ["ITEMNAME"]
        }

        //回调函数
        function selectCallBack(data) {
            debugger
            row.DSTYPE = data.GUID;
            $(cell).find('span').text(data.ITEMNAME);
            //修改资源
        ...
        $DS.openCfgTree(cfg, taskPageInfo.DSTYPETREE, selectCallBack, '40%', '55%', '设置资源分类');
    }
}
...
function registerSource(data) {
    ...
    for (let item of data['inserted']) {
        if (item.DSTYPE) {
            let newItem = {};
            newItem.DSGUID = item.GUID;
            newItem.SUPERGUID = '#';
            newItem.DSTYPE = item.DSTYPE;
            newItem.ITEMCODE = item.TASKNO;
            newItem.ITEMNAME = item.TASKNAME;
            saveData['inserted'].push(newItem)
        }
    }

    let result = $DS.saveAllTableData("SSO_T_DATARESOURCE", "DSGUID", saveData, 'PORTAL');
    if (result.isError) {
        console.error(result.errMsg ? result.errMsg : '保存资源失败');
        return false;
    }
}
...
function updateSource(data) {
    ...
    let sourceItem = {
        SUPERGUID: '#',
        DSGUID: data.GUID,
        ITEMCODE: data.TASKNO,
        ITEMNAME: data.TASKNAME,
        DSTYPE: data.DSTYPE,
    }
...
//删除任务同时删除资源以及角色对资源
function delSourceWithDelTask(val) {
    debugger
    if (val.DSTYPE) {
        //删除资源
        let delSourceSql = `delete from SSO_T_DATARESOURCE where DSGUID in (select DSGUID from SSO_T_DATARESOURCE t start with t.GUID = (select GUID from SSO_T_DATARESOURCE t where DSGUID = '${val.GUID}') connect by prior t.guid = t.SUPERGUID)`;
        //删除角色对资源
        let delRoleSql = `delete from SSO_T_ROLERESOURCE where RESOURCEID in(select GUID from SSO_T_DATARESOURCE t start with t.GUID = (select GUID from SSO_T_DATARESOURCE t where DSGUID = '${val.GUID}') connect by prior t.guid = t.SUPERGUID)`;
        let res = $DS.exeSqls(delSourceSql + ";" + delRoleSql, 'PORTAL');
        if (res.isError) {
