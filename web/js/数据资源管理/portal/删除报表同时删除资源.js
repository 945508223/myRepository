...
function deleteSource(val) {
    debugger
    if (val.DSTYPE) {
        let delSourceSql = `delete from SSO_T_DATARESOURCE where DSGUID = '${val.GUID}' and DSTYEP = '${val.DSTYPE}'`;
        let delRoleSql = `delete from SSO_T_ROLERESOURCE where RESOURCEID = '${val.GUID}'`;
...
function dropdown_Import_click(command) {
    ...
    if (command == "1") {//新增导入
        ...
        $DS.showPage("freeFromView.jsp?PAGEID=179D7FCBCBA549829FD9F3765DEE5D21&PAGETITLE=【8-0-C】报表导入&APPID=BMP&treeId=" + $DS.getPms("P_RPTFL")[0].GUID, "新增报表导入", "60%", "80%");
        ...
        //     return;
        // }
        $DS.showPage("freeFromView.jsp?PAGEID=179D7FCBCBA549829FD9F3765DEE5D21&PAGETITLE=【8-0-C】报表导入&APPID=BMP&treeId=" + $DS.getPms("P_RPTFL")[0].GUID, "覆盖报表导入", "60%", "80%");
    }
}
