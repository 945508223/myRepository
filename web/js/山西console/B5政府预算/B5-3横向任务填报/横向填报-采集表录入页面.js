...
function btn_cesg_click() {
    ...
    var TASKID = $DS.getPms("SUPERTASKID");
    var op = $DS.getPms("op");
...
function btn_loadOtherData_complete() {
    ...
    obj.ds_show = false;
    //var subWin = $iframe.getSubWindow("IFRAME_采集表");
    if ((taskStatus == "1" || taskStatus == "21" || taskStatus == "112" || taskStatus == "113" || taskStatus == "11") && myParent == "isFill")
        obj.ds_show = true;
}
...
//导出
function batch_export_buildData(carryBkColor) {
    debugger
    $DS.util.exportReportExcel("IFRAME_采集表", {backColor: carryBkColor == "0" ? false : true})
