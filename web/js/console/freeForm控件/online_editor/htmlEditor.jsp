...
<div class="tool-group">
    <span style="margin-right: 15px;">HTML在线编辑器</span>
    ...
    <div id="right_btn" style="margin-left: 58%">
...
<script type="text/javascript">
    ...
    //GUID:加载页面html,如果为页面级html需要从数据库中取
    var guid = YCDCommon.Win.getUrlParam("GUID");
    //ID:加载事件html,事件html直接从当前页面id对应的对象中找
    ...
    var html_Text =(id)?parent.VUECFG.formObj[id].info[name]:null;

    if(guid){
        getJsValueByGuid()
    }else {
        if(html_Text)
            html_editor.setValue(html_Text);    //给代码框赋值
    }
    ...
    //查询页面脚本html
    function getHtmlValueByGuid(){
        debugger
        var col="GUIDHTML_TEXT";
        if(type==="pubhtml"){
        ...
        if(data){
            ptype="edit"
            var base = new Base64();
            var htmlValue=(data.HTML_TEXT)?base.decode(data.HTML_TEXT):"";
            if(htmlValue&&htmlValue!=""){
                html_editor.setValue(htmlValue);    //给代码框赋值
            }
        }
    }
    //保存html
    function save_html(){
        debugger
        var base = new Base64();
        var htmlValue=html_editor.getValue();
        if(guid&&type==="private"){
            ...
            var datas={
                ITEMCODE:guid,
                ITEMNAME:pageName,
                HTML_TEXT:base.encode(htmlValue),
                TYPE:type
            };
            if(ptype=="edit"){
            ...
            parent.$DS.saveTable(parent.VUECFG.appId,ptype,datas,"DM_FORM_PAGEJS","GUID",function (data) {
                debugger
                parent.$DS.setPro("PAGE","ds_pagehtml","htmleditor", "【脚本】")
                close_();
                parent.alert("保存成功!")
            })
        }else if(guid&&type==="pubhtml"){
            debugger
            var datas={
                ITEMCODE:YCDCommon.Win.getUrlParam("ITEMCODE"),
                ITEMNAME:YCDCommon.Win.getUrlParam("ITEMNAME"),
                JS_TEXT:base.encode(htmlValue),
                TYPE:"pubhtml",
                GUID:guid
            };
            ...
            parent.$DS.setPro(id,name,(ispro=="true")?"htmleditor":"input",htmlValue);
            close_();
    ...
    function selectTemplate(e) {
        ...
        setCommonTemplete(html_editor,value)
    }
</script>
<!--模板1-->
