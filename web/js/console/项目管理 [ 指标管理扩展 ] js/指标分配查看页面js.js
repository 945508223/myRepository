...
function cellInputCheck(val) {
    ...
    if (val.value && val.value[val.value.length - 1] != "." && val.value != "0" && val.value != "0.0" && val.value != "0.00") {
        //var reg = /^(?!0+(?:\.0+)?$)(?:[1-9]\d*|0)(?:\.\d{1,2})?$/;//限制只能为数字并且数字最多带2位小数
        var reg = /^[+-]?\d*\.?\d{0,2}$/;
        ;
    ...
    for (var key in pageObj.editBeforeData) {
        for (let i = 0; i < listGridData.length; i++) {
            if (pageObj.editBeforeData[key].SUPERGUID === listGridData[i].GUID && computedRows.indexOf(listGridData[i].GUID) === -1) {
                var flag = checkChildrenHaveData(listGridData[i].children);
                if(flag==true){
                   continue;
                }
...
function checkChildrenHaveData(data) {
    const values = data.map(item => item.MONEY);
...
function checkSetMoney(money, rowId) {
    //var reg = /^(\.*)(\d+)(\.?)(\d{0,2}).*$/g;//限制只能为数字并且数字最多带2位小数
    var reg = /^[+-]?\d*\.?\d{0,2}$/;
...
function saveZBFPGridData() {
    ...
    var delSql = "delete from RURAL_INDI_DETAIL where INDI_ID = " + "'" + pageObj.guid + "'";
    if (!delRes.isError) {
        ...
        newEditData.inserted = editData.updated.map(item => {
            ...
            newItem["DIS_AGENCYCODE"] = item.GUID;
            return newItem;
        });
