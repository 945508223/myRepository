...
function isShowFUNDCLASSDETAIL(val) {
    var ctrlInfo = $DS.getCtrl("SELECT_财政资金分类明细").info;
    //初始化修改
    if (val == "init" || val == "1D99F29F80B64D0B8C1FCB7C9FBFE94A") {
        ctrlInfo.ds_show = true;
        ctrlInfo.ds_isrequired = true;
    } else {
        ctrlInfo.ds_show = false;
        ctrlInfo.ds_isrequired = false;
    }
}
...
function savedata() {
    ...
    if (result.isError) {
        ...
        if (res.isError || res == false) {
            ...
            debugger
            flashRightTabGrid(money, result.result.rowObj.GUID);
            close_();
            parent.alert("保存成功!");
...
//保存成功 刷新右侧tab页表格
function flashRightTabGrid(money, id) {

    //刷新右侧表格
    var tabId = parent.$DS.getCtrl("TAB").info.ds_id;
    parent.$("#" + tabId).find("iframe").eq(0)[0].contentWindow.winload(id);
    if (parent.$("#" + tabId).find("iframe").eq(1) && parent.$("#" + tabId).find("iframe").eq(1)[0]) {
