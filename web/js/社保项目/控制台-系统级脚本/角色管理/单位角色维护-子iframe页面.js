...
let pageInfo_ = {
    sourceFilter:{
        pubRole:"and ADMDIV='****' and IS_PUBROLE = '1'",
        normalRole:"[AND] (  ADMDIV='****'  or ADMDIV ='${V.P_ADMDIV.0.GUID,\"\"}'  ) [AND] ROLETYPE='2' [AND] YEAR='${V.USER_CURRENTYEAR,\"\"}'"
    },
...
function init_() {
    debugger
    let tabType = parent.$DS.getPms('URL_roleType');
    if(tabType=='2'){
        let  roleType = parent.$DS.getPms('roleType');
        //根据角色类型不同设置不同数据源过滤条件
        let source = $DS.getSource('DS_角色视图')
        if(!roleType||roleType=='0'){
            pageInfo_.isPubRole=true
            source.filter = pageInfo_.sourceFilter.pubRole;
        }else{
            pageInfo_.roleType=false
            source.filter = pageInfo_.sourceFilter.normalRole;
        }
    }
}
init_();
